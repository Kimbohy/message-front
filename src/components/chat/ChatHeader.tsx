...
export function ChatHeader({ activeChat }: ChatHeaderProps) {
  ...
  return (
    <header className="h-[60px] flex items-center gap-4 px-6 border-b border-wp-border bg-wp-header-bg">
      ...
      <div className="flex-1">
        <h2 className="font-medium text-wp-text-primary text-[17px] leading-tight">
          {name}
        </h2>
        <p className="text-[13px] text-wp-text-secondary leading-tight">
          {isOnline && activeChat.type === "PRIVATE" ? (
